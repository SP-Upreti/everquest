...
import Bhutanpackages from "./Bhutanpackages";
...
function ActivityDetail({ activity, activiti }: Props) {
  console.log("activiti", activiti);
  return (
    <div>
      <div className="lg:w-11/12 w-11/12 3xl:w-8/12 mx-auto md:mt-20 mt-10">
            ...
            <div className="col-span-3">
              <div className="grid grid-cols-2 gap-4">
                {activiti.images ? (
                  <>
                    ...
                    {activiti.images
                      ...
                      .map((image: any, i: number) => (
                        <div
                          key={i}
                          className={`relative w-full overflow-hidden rounded-lg group max-h-[15em] ${
                            i === 2 ? "col-span-2" : ""
                          }`}
                        >
                          <Image
                  ...
                  <>
                    {activiti.image.slice(0, 3).map((image: any, i: number) => (
                      <div
                        key={i}
                        className={`relative w-full overflow-hidden rounded-lg group max-h-[15em] ${
                          i === 2 ? "col-span-2" : ""
      ...
      {activiti?.collections?.slug === "tour" ? <Bhutanpackages /> : <></>}
    </div>
  );
}
